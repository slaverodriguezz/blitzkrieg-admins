...
function main()
    ...
    sampAddChatMessage(string.format("{%s}[blitzkrieg] {FFFFFF}Чеккер читеров загружен. v%d | Команды: /stream, /maxlvl, /sfc | Автор: {7B70FA}slave_rodriguez", mainColor, script_version), -1)
    ...
    sampRegisterChatCommand("sfc", function()
        ...
        if #ids > 0 then
            ...
            sampAddChatMessage(string.format("{%s}[blitzkrieg] {FFFFFF}Читеров нет (или у всех твой клист).", mainColor), -1)
        end
    end)
    ...
    sampRegisterChatCommand("stream", function()
        ...
        if count == 0 then 
            sampAddChatMessage(string.format("{%s}В зоне стрима нет подходящих игроков (кроме своих).", mainColor), -1) 
        end
    end)
